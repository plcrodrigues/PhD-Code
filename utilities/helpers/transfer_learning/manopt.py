...
# @pymanopt.function.autograd(SpecialOrthogonalGroup)
...
def egrad_function_pair_rie(M, Mtilde, Q):
    ...
    return 4 * np.dot(np.dot(Mtilde_invsqrt, logm(term_aux)),
                      np.dot(M_sqrt, Q))
...
def get_rotation_matrix(M, Mtilde, weights=None, dist=None):
    ...
    if dist == 'euc':
        cost = partial(cost_function_full, M=M, Mtilde=Mtilde,
                       weights=weights, dist=dist)
        problem = Problem(manifold=manifold, cost=cost)
    elif dist == 'rie':
        cost = partial(cost_function_full, M=M, Mtilde=Mtilde,
                       weights=weights, dist=dist)
        egrad = partial(egrad_function_full_rie, M=M, Mtilde=Mtilde,
                        weights=weights)
